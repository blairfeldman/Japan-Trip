...
export default function MoneyScreen() {
  ...
  const jpyTotal = useMemo(() => BUDGET_ITEMS.reduce((s, i) => s + i.jpy, 0), []);
  ...
  const todayJpy = todayItems.reduce((s, i) => s + i.jpy, 0);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
      ...
      <SectionLabel style={{ marginBottom: 4 }}>Today · Day {day.day}</SectionLabel>
        ...
        {todayItems.map((c) => (
          <View key={c.id} style={styles.costRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.costTagText, { color: c.tag === 'Prepaid' ? COLORS.accentTintText : COLORS.payThereText }]}>{c.tag.toUpperCase()}</Text>
            </View>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={styles.costUsd}>{usd(c.usd)}</Text>
              <Text style={styles.costJpy}>{jpy(c.jpy)}</Text>
            </View>
          </View>
        ))}
